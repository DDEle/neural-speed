...
#pragma unroll
            for (uint32_t sub_block_y = 0;
                ...
                const uint32_t sub_block_offset_x
                        = payload.base_x + offset_x + 0;
                const uint32_t sub_block_offset_y
                        = payload.base_y + offset_y + sub_block_y;
                const auto offset_ch_dim = payload_t::trans
                        ? sub_block_offset_x
                        : sub_block_offset_y;
                const auto size_ch_dim = payload_t::trans
                        ? payload.width_in_elems
                        : payload.height_in_elems;

                xetla_mask<num_channel> pred
                        = offset_ch_dim + num_channel > size_ch_dim
                        ? (xetla_vector_gen<uint32_t, num_channel>(
                                   offset_ch_dim, 1)
                                < size_ch_dim)
                        : 1;
