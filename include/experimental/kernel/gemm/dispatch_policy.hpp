...
/// A special GEMM implementation to increase the hardware occupancy by
/// splitting the GEMM task along k dimension. It includes inter-group reduction
/// (by using global atomic) and intra-group reduction (by using local memory
/// for data exchange).
/// @note The difference compare with dispatch_policy_kslicing is we will add
/// additional handling for 4bit.
