...
#include <memory>
#include "ifmha_forward.hpp"
#include "ifmha_forward_policy.h"
...
constexpr uint32_t num_beams = 4;
constexpr uint32_t num_queries = 1; // must be 1
constexpr uint32_t num_kv0 = 1919;
constexpr uint32_t num_kv1 = 111;
constexpr uint32_t max_kv = 2176;
...
void fmha_run(uint32_t iter, uint32_t warmup = 10) {
  ...
  auto Q = alloc_device_and_init<FMHA_T>(
      num_batches * num_beams * num_heads * head_size * num_queries,
      ...
      device,
      context);
  auto K0 = alloc_device_and_init<FMHA_T>(
      num_batches * num_heads * head_size * num_kv0,
      [](FMHA_T* data, size_t idx) {
        data[idx] = static_cast<FMHA_T>(random_float());
      },
      queue,
      device,
      context);
  auto V0 = alloc_device_and_init<FMHA_T>(
      num_batches * num_heads * head_size * num_kv0,
      [](FMHA_T* data, size_t idx) {
        data[idx] = static_cast<FMHA_T>(random_float());
      },
      ...
      device,
      context);
  auto K1 = alloc_device_and_init<FMHA_T>(
      max_kv * num_batches * num_beams * num_heads * head_size,
      [](FMHA_T* data, size_t idx) {
        data[idx] = static_cast<FMHA_T>(random_float());
      },
      queue,
      device,
      context);
  auto V1 = alloc_device_and_init<FMHA_T>(
      max_kv * num_batches * num_beams * num_heads * head_size,
      [](FMHA_T* data, size_t idx) {
        data[idx] = static_cast<FMHA_T>(random_float());
      },
      ...
      device,
      context);
  auto IDX = alloc_device_and_init<index_t>(
      num_kv1 * num_batches * num_beams,
      [](index_t* data, size_t idx) {
        static std::random_device dev;
        static std::mt19937 rng(dev());
        static std::uniform_int_distribution<int> dist(0, num_beams - 1);
        data[idx] = static_cast<index_t>(dist(rng));
      },
      queue,
      device,
      context);
    ...
    auto gpu_event = queue.submit([&](handler& cgh) {
      cgh.parallel_for(nd_range, [=](sycl::nd_item<3> item) KERNEL_MAIN {
        typename ifmha_forward_op_t::arguments_t kern_args(
            ...
            0,
            0);
        ifmha_forward_op_t{}(item, kern_args);
      });
    });
    gpu_event.wait();
