...
void sdp_fwd_run(uint32_t iter, uint32_t warmup = 10) {
    ...
    const auto max_wi_sizes
            = device.get_info<info::device::max_work_item_sizes<3>>();
    std::cout << "max_work_item_sizes: " << max_wi_sizes[0] << " "
              << max_wi_sizes[1] << " " << max_wi_sizes[2] << "\n";
    std::cout << "sub_group_sizes:";
    const auto d_sg_sizes = device.get_info<info::device::sub_group_sizes>();
    for (const auto sg_size : d_sg_sizes) {
        std::cout << " " << sg_size;
    }
    std::cout << "\n";
    if (device.has(aspect::ext_intel_gpu_subslices_per_slice)) {
        auto subslices = device.get_info<
                ext::intel::info::device::gpu_subslices_per_slice>();
        std::cout << "gpu_subslices_per_slice: " << subslices << "\n";
    }
    if (device.has(aspect::ext_intel_gpu_eu_count_per_subslice)) {
        auto euCount = device.get_info<
    ...
    auto gpu_kernel = [=](nd_item<3> item) KERNEL_MAIN {
        using namespace gpu::xetla;
        ...
        using elem_opt_mode_t = elem_v_t<tune_key::param_optimizer_mode,
                param_optimizer_mode::keep_shape>;
        using elem_opt_type_t = elem_v_t<tune_key::param_optimizer_type,
        ...
        using epilogue0_t = xetla::group::default_epilogue_selector_t<
                ...
                tune_option0>;
        using gemm_op0_t
                = gemm_universal_t<dispatch_policy_default<group_swizzle>,
                        gemm0_t, epilogue0_t>;
        ...
        typename gemm_op0_t::arguments_t arg0(matrix_m_qk, matrix_k_qk,
                matrix_n_qk,
        ...
        using softmax_op_t = xetla_softmax_fwd_t<dtype_sfx, dtype_in,
                tile_shape0, mem_space::local, mem_space::local, SIMD,
                thread_num, softmax_sz, arch_tag>;
        ...
        using matC_t = tile_t<dtype_out,
                tile_desc_t<matAcc_t::tile_size_x, matAcc_t::tile_size_y,
        ...
        dtype_in *matB_ptr = v + batch_id * size_qkv; // matB_ptr + batch offset
        ...
        const uint32_t batch_offset = b * head_num * sequence_len * head_size
        ...
        mem_desc_c.init(out + batch_offset, // dst_base = out_ptr + wg offset
                {std::min(h + sg_tile_n_sv, wg_tile_n_sv),
                        std::min(f + sg_tile_m_sv, wg_tile_m_sv), ld_c},
                {int(h), int(f)});
