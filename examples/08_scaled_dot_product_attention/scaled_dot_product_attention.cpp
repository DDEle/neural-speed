...
void sdp_fwd_run(uint32_t iter) {
    ...
    auto gpu_kernel = [=](nd_item<3> item) KERNEL_MAIN {
        ...
        using post_op1_t
                = elemwise_reduce_op_t<reduce_op::sum, dtype_in, gpu_arch::Dg2>;
        ...
        xetla_nbarrier_t<thread_num, thread_num, gpu_arch::Dg2> nbarrier;
        ...
        for (uint32_t j = 0; j < sg_tile_m_sv; ++j, ++f) {
            uint32_t dst_offset = b * head_num * sequence_len * head_size
                    + f * head_num * head_size + n * head_size;
            out_reg = matC.reg.xetla_select<sg_tile_n_sv, 1>(j * sg_tile_n_sv);
            xetla_fill_tdesc<dtype_out, sg_tile_n_sv, 1, 1>(
                    transpose_tdecs.xetla_format<uint32_t>(), out + dst_offset,
                    head_size, 1, head_size, h, 0);
            xetla_tstore_global<dtype_out, sg_tile_n_sv, cache_hint::write_back,
                    cache_hint::write_back, gpu_arch::Dg2>(
    ...
    try {
        for (uint32_t i = 0; i < iter + warmup; i++) {
            ...
            auto gpu_event = queue.submit([&](handler &cgh) {
                cgh.parallel_for<class Test>(nd_range, gpu_kernel);
            });
            gpu_event.wait();
