...
using FMHA_T = fp16;
// using policy_t = gpu::xetla::fmha_policy_8x256x256;
using policy_t = gpu::xetla::fmha_policy_64x128x128;

constexpr uint32_t num_batches = 1;
constexpr uint32_t num_heads = 14;
constexpr uint32_t head_size = 128;
constexpr uint32_t num_queries = 1023;
constexpr uint32_t num_keys = 1023;
...
int fma_result_validate(FMHA_T *q_device, FMHA_T *k_device, FMHA_T *v_device,
                        FMHA_T *DST_device, sycl::queue &queue) {
  ...
  std::vector<accum_t> gold_SP(num_batches * num_heads * num_queries * num_keys,
                               0);
  for (uint32_t gid = 0; gid < num_batches * num_heads; gid++) {
    ...
    for (uint32_t i = 0; i < num_queries; ++i)
      std::copy_n(Q_cur +
                      i * head_size * num_heads * (kSeqLast ? num_batches : 1),
                  head_size, Q_tmp.get() + i * head_size);
    ...
    get_gemm_gold<FMHA_T, FMHA_T, accum_t>(
        num_queries, num_keys, head_size, mem_layout::row_major,
        mem_layout::row_major, Q_tmp.get(), K_tmp.get(), gold_cur);
  ...
  for (uint32_t gid = 0; gid < num_batches * num_heads; gid++) {
    ...
    for (uint32_t i = 0; i < num_keys; ++i)
      std::copy_n(V_cur +
                      i * head_size * num_heads * (kSeqLast ? num_batches : 1),
                  head_size, V_tmp.get() + i * head_size);
    get_gemm_gold(num_queries, head_size, num_keys, mem_layout::row_major,
                  mem_layout::row_major, P_cur, V_tmp.get(), dst_cur.get());

    // permute 0213
    const auto gold_cur = gold_DST.data() +
                          batch_id * num_queries * num_heads * head_size +
                          head_id * head_size;
    for (uint32_t i = 0; i < num_queries; ++i)
      std::copy_n(dst_cur.get() + i * head_size, head_size,
                  gold_cur +
                      i * num_heads * head_size * (kSeqLast ? num_batches : 1));
  }
  buff_cmp::buff_vals<FMHA_T> data( //
      DST_ptr, num_queries * num_heads * num_batches, head_size, head_size);
  buff_cmp::buff_vals<FMHA_T, accum_t> other(
      gold_DST.data(), num_queries * num_heads * num_batches, head_size,
      head_size);
...
void fmha_run(uint32_t iter, uint32_t warmup = 10) {
  using fmha_forward_op_t =
      gpu::xetla::fmha::fmha_forward_t<policy_t, FMHA_T, gpu_arch::Igpu, false,
                                       false, false, kSeqLast, false, false>;
    ...
    auto gpu_event = queue.submit([&](handler &cgh) {
      cgh.parallel_for(nd_range, [=](sycl::nd_item<3> item) KERNEL_MAIN {
        typename fmha_forward_op_t::arguments_t kern_args(
            Q, K, V, nullptr, nullptr, nullptr, DST, L, num_batches, num_heads,
            num_heads, // num_kv_heads
            head_size, num_queries, num_keys, -1, -1, -1, softmax_scale, 0, 0,
  ...
  auto DST_ptr = alloc_host_and_copy<FMHA_T>(
      DST, num_batches * num_heads * head_size * num_queries, queue);
  free(Q, context);
  free(K, context);
  free(V, context);
  free(DST, context);
