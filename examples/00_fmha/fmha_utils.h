...
struct bias_add_op_t<dtype_bias_, gpu_arch::Xe> {
  ...
  operator()(matAcc_t &matAcc, const coord_t &coord, const arguments_t &args,
    ...
    using bias_tile_desc_t = subgroup::tile_desc_t<tile_size_x, 1, block_size_x,
                                                   1, reg_layout::tiled>;
...
#pragma unroll
      for (int j = 0; j < num_block_x; j++) {
        auto dst_reg =
            matAcc.reg
                .xetla_select<tail_block_elems, 1>(tail_start_y * tile_size_x +
                                                   j * tail_block_elems)
