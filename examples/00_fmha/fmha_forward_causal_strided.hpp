// Kernel template in a seperate file so it can be tested and debugged
// (with xxx) individually
...
template <typename fmha_policy, typename scalar_t, gpu_arch arch_tag,
        bool kUseAlibi, bool kUseBias, bool kIsCausal, bool kSeqLast,
        bool kIsTraining>
...
private:
    ...
    using compute_policy = std::conditional_t<(arch_tag >= gpu_arch::Dg2),
    ...
    static_assert(
            wg_size <= 32, "The number of threads should be less than 32!");
    ...
    struct context_t {
        ...
        inline void init_context(nd_item<3> &item, arguments_t &args) {
            ...
            if constexpr (kSeqLast) { // 2d mem: [F, BxNxH]
                ...
                mem_desc_Qi.init(args.Q_ptr, {end_x, end_y, b_stride * args.uB},
                        {start_acc, start_y});
                mem_desc_Oi.init(args.O_ptr, {end_x, end_y, b_stride * args.uB},
                        {start_acc, start_y});
            } else { // 2d mem: [BxF, NxH]
                // startF
                ...
                mem_desc_Qi.init(args.Q_ptr, {args.uH * args.uN, end_y, ld_qo},
                        {start_acc, start_y});
                mem_desc_Oi.init(args.O_ptr, {args.uH * args.uN, end_y, ld_qo},
                        {start_acc, start_y});
            }
        ...
        inline void update_context(
                nd_item<3> &item, arguments_t &args, uint32_t startT) {
            uint32_t gid = item.get_group(0);
            uint32_t batch_id = gid / args.uN; // get batch idx
            uint32_t head_id = gid % args.uN; // get head idx
            ...
            if constexpr (kSeqLast) {
                ...
                mem_desc_Kj_T.init(args.K_ptr,
                        {end_x, end_y, b_stride * args.uB},
                        {start_x, start_acc});
                mem_desc_Vj.init(args.V_ptr, {end_y, end_x, b_stride * args.uB},
                        {start_acc, start_x});
            } else {
            ...
            if constexpr (kUseAlibi) {
                ...
                mem_desc_Ai.init(args.A_ptr,
                        {end_x, 1, args.uAT * args.uN * args.uB}, {start_x, 0});
            }
            ...
            if constexpr (kUseBias && kIsCausal) {
                ...
                mem_desc_Bij.init(args.B_ptr, {end_x, 1, args.uMT * args.uB},
                        {start_x, 0});
            }
        }
    };
    ...
    inline void gemm_Sij(matAccSij_t &matAccSij, arguments_t &args) {
        ...
        brgemm_args_t brgemm_args(
                ctx.mem_desc_Qi_L, ctx.mem_desc_Kj_T, loop_count);
        brgemm(ctx.g, matAccSij, brgemm_args, 0,
        ...
        if constexpr (kUseAlibi) {
            using alibi_op_t = subgroup::bias_add_op_t<
                    mem_desc_t<scalar_t, mem_layout::row_major,
                            mem_space::global>,
                    arch_tag>;
            ...
            alibi_op_t alibi_op;
            alibi_args_t alibi_args(
                    ctx.mem_desc_Ai.base, ctx.mem_desc_Ai.shape);
            alibi_op(matAccSij, ctx.mem_desc_Ai.coord, alibi_args);
        }

        // Add attn_mask if needed
        if constexpr (kUseBias && !kIsCausal) {
            using bias_op_t = subgroup::elemwise_reduce_op_t<reduce_op::sum,
                    scalar_t, arch_tag>;
            ...
            bias_args_t bias_args(
                    ctx.mem_desc_Bij.base, ctx.mem_desc_Bij.shape);
    ...
    using matOi_t = tile_t<scalar_t,
            tile_desc_t<matAccOi_t::tile_size_x, matAccOi_t::tile_size_y,
                    matAccOi_t::block_size_x, matAccOi_t::block_size_y,
                    reg_layout::tiled>>;

    /// @brief gemm_Oi is used to compute Oi += Pij x Vj
    /// # [Br,Bc] x [Bc,H] = [Br,Hm]
    inline void gemm_Oi(
            matAccOi_t &matAccOi, arguments_t &args, uint32_t startT) {
        ...
        brgemm_Oi_t brgemm;
        brgemm_args_t brgemm_args(
                ctx.mem_desc_Pij_L, ctx.mem_desc_Vj, loop_count);
        brgemm(ctx.g, matAccOi, brgemm_args, 0,
                /* nbarrier_base */ nbarrier_cnt);
    }
    ...
    inline void apply_mask(matAccSij_t &matAccSij, arguments_t &args,
        ...
        if (remainT < kSgBc) { tile_mask::padding_mask(matAccSij, remainT); }
    ...
    inline void softmax_fwd(matAccSij_t &matAccSij, matAccOi_t &matAccOi,
        ...
        using wg_row_max_t = group_row_reduce_t<matAccSij_t, wg_size_x,
                reduce_op::max, arch_tag>;
        using wg_row_sum_t = group_row_reduce_t<matAccSij_t, wg_size_x,
                reduce_op::sum, arch_tag>;

        // init slm address for group reducer
        uint32_t reducer_slm = softmax_slm
                + ctx.sg_idy * wg_size_x * kSgBr * sizeof(accum_t);
        ...
        (matAccSij.reg)
                .xetla_merge(
                        mat_zeros.reg, xetla_exp<accum_t>(matAccSij.reg), mask);
        ...
        using epilogue_t
                = group::epilogue_t<group::epilogue_policy_default<arch_tag>,
    ...
    inline void raw_store_Oi(
            matAccOi_t &matAccOi, [[maybe_unused]] arguments_t &args) {
        using epilogue_t
                = group::epilogue_t<group::epilogue_policy_default<arch_tag>,
    ...
    inline void permute_store_Oi(
            nd_item<3> &item, matAccOi_t &matAccOi, arguments_t &args) {
        ...
        if constexpr (arch_tag != gpu_arch::Xe) {
            ...
            mem_desc_Oi.init(
                    ...
                    {std::min(kSgBc, args.uH - h), std::min(kSgBr, args.uF - f),
                            ld_c},
                    {0, 0});
            using matOi_tile_desc_t = typename matOi_t::tile_desc;
            using matOi_store_t = subgroup::mem_payload_t<
                    mem_desc_t<scalar_t, mem_desc_Oi_t::layout,
                            mem_desc_Oi_t::space>,
                    matOi_tile_desc_t,
                    subgroup::msg_type_v<matOi_tile_desc_t,
                            mem_desc_Oi_t::space>,
                    arch_tag>;
            matOi_store_t matOi_store(mem_desc_Oi);
            subgroup::tile_store<cache_hint::write_back,
                    cache_hint::write_back>(matOi, matOi_store);
            return;
        }

        xetla_fill_tdesc<scalar_t, kSgHm, 1, 1>(
                transpose_tdecs.xetla_format<uint32_t>(), args.O_ptr, args.uH,
                height, args.uH, h, offset_height);
...
public:
    ...
    static sycl::nd_range<3> get_nd_range(
            uint32_t total_batches, uint32_t num_queries) {
        // local range
