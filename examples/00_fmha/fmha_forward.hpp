...
template <typename fmha_policy, typename scalar_t, gpu_arch arch_tag,
          ...
          bool kIsTraining, bool kIsDropout>
...
public:
  ...
  struct arguments_t {
    ...
    inline arguments_t(scalar_t *query, scalar_t *key, scalar_t *value,
                       ...
                       scalar_t *out, accum_t *logsumsoftmax,
                       uint32_t num_batches, uint32_t num_heads,
                       uint32_t num_kv_heads, uint32_t head_size,
                       uint32_t num_queries, uint32_t num_keys,
                       uint32_t bias_strideB, uint32_t bias_strideN,
                       uint32_t bias_strideF, accum_t sm_scale,
                       accum_t dropout_prob, uint32_t alibi_padded_block_size,
                       uint32_t attn_mask_padded_block_size, uint64_t seed_t,
                       uint64_t offset_t)
        : Q_ptr(query), K_ptr(key), V_ptr(value), A_ptr(alibi), B_ptr(bias),
          Dp_ptr(dropout), O_ptr(out), L_ptr(logsumsoftmax), uB(num_batches),
          uN(num_heads), uNkv(num_kv_heads), uH(head_size), uF(num_queries),
          uT(num_keys), sm_scale(sm_scale), bias_strideB(bias_strideB),
          bias_strideN(bias_strideN), bias_strideF(bias_strideF),
          dp_prob(dropout_prob), dp_scale(1.f / (1.f - dropout_prob)),
          uAT(alibi_padded_block_size), uMT(attn_mask_padded_block_size),
          seed(seed_t), offset(offset_t), is_bias_add(bias_strideF == 0) {}
  };
...
private:
  ...
  using gemm_Sij_t = group::gemm_t<compute_policy_BrBc, tile_shape_BrBc,
  ...
  struct context_t {
    ...
    inline void update_context(nd_item<3> &item, arguments_t &args,
                               uint32_t startT) {
      uint32_t gid = item.get_group(0);
      uint32_t batch_id = gid / args.uN; // get batch idx
      uint32_t head_id = gid % args.uN;  // get head idx
      ...
      if constexpr (kSeqLast) {
        ...
        mem_desc_Kj_T.init(args.K_ptr, {end_x, end_y, b_stride * args.uB},
                           {start_x, start_acc});
      ...
      if constexpr (kUseAlibi) {
        ...
        mem_desc_Ai.init(args.A_ptr, {end_x, 1, args.uAT * args.uN * args.uB},
                         {start_x, 0});
      }
  ...
  using gemm_Oi_t = group::gemm_t<compute_policy_BrBm, tile_shape_BrHm,
                                  mem_desc_Pij_L_t, mem_desc_Vj_t>;
  using matAccOi_t = typename gemm_Oi_t::matAcc_t;
  using matOi_t = subgroup::tile_t<
      scalar_t,
      subgroup::tile_desc_t<matAccOi_t::tile_size_x, matAccOi_t::tile_size_y,
  ...
  inline void store_for_backward([[maybe_unused]] const arguments_t &args) {
    ...
    using store_payload_t = subgroup::mem_payload_t<
        mem_desc_t<accum_t, mem_layout::row_major, mem_space::global>,
        store_desc, msg_type::block_2d, arch_tag>;
  ...
  inline void rescale_then_store_Oi(matAccOi_t &matAccOi,
                                    [[maybe_unused]] arguments_t &args) {
    subgroup::tile_broadcast_op<tile_mul, matAccOi_t>(matAccOi,
                                                      1 / ctx.softmax_l);
    using epilogue_t =
        group::epilogue_t<group::epilogue_policy_default<arch_tag>,
  ...
  inline void permute_store_Oi(nd_item<3> &item, matAccOi_t &matAccOi,
                               arguments_t &args) {
